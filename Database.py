...
class Connector:
    ...
    def master(self):
        ...
        self.session.execute("CREATE TABLE trial(id int PRIMARY KEY,Airline text,Source text,Destination text,Total_Stops text,Total_Duration int,Journey_month int,Journey_day int);")
    ...
    def addData(self, result):
        ...
        value = "{0},'{1}','{2}','{3}','{4}',{5},{6},{7}".format(4, result['Airline'], result['Source'],
                                                                 result['Destination'], result['Total_Stops'],
                                                                 result['Total_Duration'], result['Journey_month'],
                                                                 result['Journey_day'])
        logger.info("String created")
        custom = "INSERT INTO trial({}) VALUES({});".format(column, value)
    ...
    def getData(self):
        self.session.execute("use flighpricedata")
        row = self.session.execute("SELECT * FROM trial;")
        collection = []
        for i in row:
